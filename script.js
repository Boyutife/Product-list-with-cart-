...
function renderProducts(data) {
  data.forEach((product) => {
    ...
    el.innerHTML = `
      <h2 class="text-slate-900 text-xl font-semibold md:text-sm">${product.name}</h2>
      <h3 class="-order-1 text-[hsl(14,86%,42%)] md:text-sm">${product.category}</h3>
      <strong class="text-rose-500 font-semibold text-2xl md:text-sm">$${product.price.toFixed(
        2,
      )}</strong>
  ...
  />
        ...
        <div class="cartBtn absolute -bottom-6 md:-bottom-4 cursor-pointer left-1/2 transform -translate-x-1/2 bg-white border border-rose-600 w-1/2 px-3 md:px-1 py-2 flex justify-center items-center gap-2 rounded-3xl" data-id="${
          ...
          <p class="text-base md:text-xs">Add to Cart</p>
        </div>
      </div>
    `;
...
function updateCartBtnUi(btn, product) {
  ...
  btn.innerHTML = `
      <button
        ...
        class="decrease-btn border border-white rounded-full h-6 w-6 md:w-4 md:h-4 flex items-center justify-center p-0.5"
      >
      ...
      <button
        ...
        class="increase-btn border border-white rounded-full h-6 w-6 md:w-4 md:h-4 flex items-center justify-center p-0.5 "
      >
...
function updateCartForm() {
     ...
     <li
      ...
      >
        <div class="cart-item-info flex flex-col gap-2.5">
          <h3 class="text-xl md:text-sm font-semibold text-red-950">${product.name}</h3>
          <div class="cart-item-price-info flex items-center gap-2">
            <span
              class="item-quantity text-[hsl(14,86%,42%)] font-bold text-lg md:text-xs"
              >${product.quantity}x</span
            >
            <span class="item-unit-price text-rose-800 text-lg md:text-sm">
        ...
        <button
          class="remove-item-btn w-4 h-4 p-0.5 border-2 border-slate-200 rounded-full flex items-center justify-center" data-id="${
            product.id
          }"
        >
          <img
  ...
  cartItemsContainer.innerHTML = `
   <h2 class="text-left w-full text-2xl md:text-xl text-[hsl(14,86%,42%)] font-bold">
    ...
    >
      <h3 class="font-semibold md:text-lg">Order Total</h3>
      <div class="order-total-price text-2xl md:text-lg font-bold">$${totalPrice.toFixed(
        2,
      )}</div>
    </section>
    ...
    >
      ...
      <p>
       ...
       <button
      class="confirm-order-btn bg-[hsl(14,86%,42%)] text-white font-bold text-xl rounded-4xl w-full py-4 md:text-lg md:py-2"
      type="submit"
    >
      Confirm Order
    </button>
  ...
  confirmOrderBtn.addEventListener('click', (e) => {
    ...
    if (cart.size > 0) {
          ...
          <li
            ...
            >
              ...
              <div class="cart-item-price-info flex flex-col items-start gap-2">
                      <h3 class="text-base md:text-sm font-bold text-red-950 truncate w-[150px]">
                      ${item.name}
                       </h3>

                <div class="flex gap-3 w-full">
                  <span
                    class="item-quantity text-[hsl(14,86%,42%)] font-bold text-base md:text-sm"
                    >${item.quantity}x</span
                  >
                  <span class="item-unit-price text-rose-900 text-base md:text-sm">
      ...
      modalContent.className =
        'absolute flex flex-col bg-white rounded-2xl p-6 gap-4 md:gap-1.5 md:w-1/4';
      modalContent.innerHTML = `
      <img
          class="w-1/6  md:w-1/12"
          src="./assets/images/icon-order-confirmed.svg"
          alt="icon-order-confirmed"
        />
        <h2 class="text-3xl md:text-xl md:font-bold font-extrabold">Order <br />Confirmed</h2>
        <p class="text-[hsl(14,86%,42%)]">We hope you enjoy your food !</p>
        <div class="order-summary bg-rose-50 p-2.5 rounded md:p-1">
        ...
        <button
          class="start-new-order-btn bg-[hsl(14,86%,42%)] text-white font-bold text-xl md:text-lg md:font-semibold md:py-2 rounded-4xl w-full py-3 cursor-pointer"
          type="button"
        >
          Start New Order
        </button>
